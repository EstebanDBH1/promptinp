...
import { Terminal, Sparkles, LogOut, Menu, X, Settings, LayoutDashboard } from 'lucide-react';
        ...
        <>
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <div className="flex items-center gap-4 border-l border-white/5 pl-8">
              {user ? (
                <div className="flex items-center gap-6">
                  <div className="flex items-center gap-3">
                    ...
                    <Link to="/settings" className="text-sm font-medium text-white hover:text-orange-500 transition-colors hidden xl:block">
                      {user.email?.split('@')[0]}
                    </Link>
                  </div>

                  <div className="flex items-center gap-4 text-sm text-zinc-400">
                    <Link to="/prompts" className="hover:text-white transition-colors flex items-center gap-1.5">
                      <LayoutDashboard className="w-4 h-4" />
                      mis prompts
                    </Link>
                    <Link to="/settings" className="hover:text-white transition-colors flex items-center gap-1.5">
              ...
              <button className="text-xs text-zinc-500 hover:text-white flex items-center gap-1 font-mono uppercase bg-zinc-900/50 px-2 py-1 rounded border border-zinc-800">
                ES <span className="text-[10px]">▼</span>
              </button>
            </div>
          </div>
                ...
                <div className="flex flex-col gap-6 ">
                  ...
                  {user ? (
                    <div className="flex flex-col gap-4">
                      ...
                      <Link to="/prompts" className="flex items-center gap-3 text-zinc-400 hover:text-white px-2 py-2 rounded-lg hover:bg-zinc-900" onClick={() => setIsMenuOpen(false)}>
                        <LayoutDashboard className="w-5 h-5" />
                        Mis Prompts
                      </Link>
                      <Link to="/settings" className="flex items-center gap-3 text-zinc-400 hover:text-white px-2 py-2 rounded-lg hover:bg-zinc-900" onClick={() => setIsMenuOpen(false)}>
                    ...
                    <div className="flex flex-col gap-4">
                      ...
                      <Link to="/login" onClick={() => setIsMenuOpen(false)}>
                        <Button variant="primary" fullWidth>empezar ahora</Button>
                      </Link>
                    </div>
                  )}
                </div>
