...
const Stat: React.FC<{ value: string; label: string; color?: string }> = ({
  ...
  <div className="text-center">
    <div className={`text-2xl md:text-3xl font-semibold mb-1 ${color}`}>
      {value}
    </div>
      ...
      try {
        ...
        if (data) {
          const mapped = data.map((p) => ({
            ...
            modelIcon:
              p.category?.toLowerCase() === "ingeniería"
                ? "💻"
                : p.category?.toLowerCase() === "marketing"
                  ? "✍️"
                  : p.category?.toLowerCase() === "imagen"
                    ? "🎨"
                    : p.category?.toLowerCase() === "código"
                      ? "⚛️"
                      : "🤖",
    ...
    <div className="min-h-screen bg-[#050505] flex flex-col font-sans selection:bg-orange-500/30">
      ...
      <PageTransition className="flex-1 flex flex-col items-center overflow-hidden w-full">
        ...
        <section className="w-full max-w-5xl mx-auto px-6 pt-16 pb-24 text-center">
          <Reveal>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-zinc-800 bg-zinc-900/50 mb-8 hover:border-zinc-700 transition-colors cursor-default font-mono font-semibold">
          ...
          <Reveal delay={100}>
            <div className="text-xs md:text-sm text-zinc-600 mb-8 font-semibold font-mono tracking-wide">
              los modelos son mediocres • los prompts VIP los hacen
              excepcionales
            </div>
          </Reveal>
          {/*
          ...
          <Reveal delay={300}>
            <h1 className="text-3xl md:text-[48px] font-semibold text-white mb-6 leading-tight tracking-tight">
            ...
            <div className="w-full max-w-[1400px] mx-auto">
              <div className="px-6 md:px-8 flex justify-between items-end mb-8 max-w-6xl mx-auto">
                <div>
                  ...
                  <p className="text-xs md:text-sm text-zinc-500 font-mono font-semibold">
                    curados por expertos en LLMs
                  </p>
                </div>
                <Link
                  to="/prompts"
                  className="text-orange-500 text-xs md:text-sm font-semibold font-mono hover:text-orange-400 flex items-center gap-1 group tracking-wide"
                >
                  ver todo{" "}
              ...
              <div className="text-center mt-2 text-xs md:text-sm text-zinc-600 font-mono font-semibold tracking-wide opacity-60">
              ...
              <div className="flex flex-col md:flex-row gap-8 items-start md:items-center">
                <div className="flex-1">
                  ...
                  <p className="text-xs md:text-sm text-zinc-400 mb-8 leading-relaxed">
                    No pierdas el tiempo probando prompts básicos. Nuestra
                    biblioteca está diseñada para profesionales que exigen
                    resultados perfectos. Acceso instantáneo a las técnicas más
            ...
            {FEATURES.map((feature, idx) => (
              <Reveal key={feature.id} delay={idx * 150}>
                <div className="bg-[#111] border border-zinc-800 p-6 rounded-xl hover:border-zinc-700 transition-colors group h-full hover-lift">
                  ...
                  <h3 className="font-semibold text-white mb-2 text-sm md:text-base font-mono">
                    {feature.title}
                  </h3>
          ...
          <div className="relative z-10 max-w-6xl mx-auto">
            <Reveal>
              <div className="text-center mb-16">
                ...
                <p className="text-xs md:text-sm text-zinc-500 font-mono font-semibold tracking-wide">
                  invierte en mejores resultados, no en más tokens
                </p>
              </div>
            </Reveal>
          ...
          <Reveal>
            <div className="text-center mb-10">
              ...
              <p className="text-xs md:text-sm text-zinc-500">
                megaprompts optimizados para cada necesidad
              </p>
            </div>
          </Reveal>
            ...
            {TASKS.map((task, idx) => (
              <Reveal key={task.id} delay={idx * 30}>
                <div
                  onClick={() =>
                    navigate(
                      `/prompts?category=${encodeURIComponent(task.label)}`,
                    )
                  }
                  className="flex items-center gap-2 px-3.5 py-2 bg-[#161618] border border-zinc-800 rounded-lg text-xs md:text-sm text-zinc-400 hover:border-zinc-600 hover:text-white transition-all cursor-pointer shadow-sm hover-lift group"
                >
            ...
            {STEPS.map((step, idx) => (
              <Reveal key={step.id} delay={idx * 150}>
                <div className="bg-[#0f0f11] border border-zinc-800 p-5 rounded-xl flex items-center gap-5 group hover:border-zinc-700 transition-all cursor-default hover:bg-[#141416]">
                  ...
                  <div>
                    <h3 className="text-sm md:text-base font-semibold text-white mb-1 group-hover:text-orange-400 transition-colors font-mono tracking-tight">
                      {step.title}
                    </h3>
                    <p className="text-xs md:text-sm text-zinc-500">
                      {step.description}
        ...
        <section className="w-full py-24 px-6 text-center border-t border-white/5 bg-gradient-to-b from-[#050505] to-[#0a0a0a]">
          <Reveal>
            <h2 className="text-2xl font-semibold text-white mb-3 font-mono tracking-tighter">
              domina la ingeniería de prompts
            </h2>
            <p className="text-xs md:text-sm text-zinc-500 mb-8 max-w-md mx-auto">
              Únete a miles de ingenieros y creadores que están construyendo el
              futuro del software.
            </p>
